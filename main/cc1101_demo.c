...
#define TX_INTERVAL 5000			// Transmit interval in milliseconds
...
static void CC_RX_task(void* arg)
{
    ...
    uint8_t Rx_fifo[FIFOBUFFER];	//FIFOBUFFER = 0x42
	...
	while(1) {
		ESP_LOGI(tag, "rx interated here!!");
        if (xQueueReceive(gdo2_evt_queue, &io_num, portMAX_DELAY)) {
        	if (io_num == GDO2) {
					...
					if (res == 1) {
						...
						rf_timecode = ((uint32_t)Rx_fifo[3] << 24) +
									  ...
									  ((uint16_t)Rx_fifo[5] <<  8) +
												 Rx_fifo[6];
						printf("rf_timecode: %d << 24 = %d\n", rf_timecode, (rf_timecode << 24));
						float tmp;
				...
				ESP_LOGI(rxtag, "after semaphoregive!!");
        	}
        }
...
static void CC_TX_task(void* arg)
{
	...
	while (1) {
		...
		time_stamp = clock();	
		// printf("Time stamp: %8x\n", time_stamp);							// generate new time stamp
		...
		uint8_t res = send_packet(My_addr, Rx_addr, Tx_fifo, Pktlen, 1);	// send package over air. ACK is received via GPIO polling
...
void app_main()
{
    ...
    gdo2_evt_queue = xQueueCreate(100, sizeof(uint32_t));
    // printf("step2\n");

	vTaskDelay(3000 / portTICK_RATE_MS);
	// printf("step3\n");
	uint8_t res = cc_setup(&My_addr, 1);
	printf("reach here res: %d\n", res);
	if (res > 0) {
        ...
        // === Start tasks ===
		xTaskCreatePinnedToCore(CC_RX_task, "CC_RX_task", 3*1024, NULL, 1, NULL, 0);
		vTaskDelay(100 / portTICK_RATE_MS);
		xTaskCreatePinnedToCore(CC_TX_task, "CC_TX_task", 3*1024, NULL, 8, NULL, 0);
	}
	else {
 ...
 // 	uint32_t time_stamp;
	...
	// 	if (res) {
	// 		n_tx_ok++;
	// 		ESP_LOGI(txtag, "[RSSI: %d  LQI: %d  CRC: %s]", last_rssi_dbm, last_lqi, ((last_crc) ? "OK" : "BAD"));
	// 	}
